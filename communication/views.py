...
STOP_STREAM = False
LAST_GLOSS = ""
CURRENT_RECORDED_SIGNS = []
CURRENT_DISPLAY_TEXT = "Searching for hands..."
...
def _gen_frames(models_dir, output_file):
    ...
    try:
        ...
        print(f"--- [Stream] Model loading failed: {e} ---")
        return

    # Try different backends for better compatibility
    cap = cv2.VideoCapture(0)
    if not cap.isOpened():
        print("--- [Stream] Default backend failed, trying DSHOW ---")
        cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)
        
    if not cap.isOpened():
        print("--- [Stream] Camera failed to open entirely ---")
        return
    ...
    try:
        while True:
            ...
            if x is not None:
                ...
                # Faster inference using model call instead of predict()
                probs = loader.model(x.reshape(1, -1), training=False).numpy()[0]
                ...
                if ready and stable_count >= 12 and current_conf >= 0.45:
            ...
            if conf_val > 0:
                # Confidence Bar
                bw = int(conf_val * 250)
                cv2.rectangle(frame, (20, 85), (20 + bw, 100), (0, 255, 0), -1)
                cv2.putText(frame, f"{int(conf_val*100)}%", (bw + 30, 98), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)

            # Sequence display
            signs_str = " -> ".join(recorded_signs[-4:])
            if recorded_signs:
                cv2.rectangle(frame, (0, h-40), (w, h), (20, 20, 20), -1)
                cv2.putText(frame, f"Seq: {signs_str}", (20, h - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
            ...
            # Small sleep to improve framerate/yield
            time.sleep(0.01)
    finally:
...
def gloss_sentence(request):
    ...
    # Optimized NLP module loading
    if _CACHED_NLP_MOD is None:
        parent_dir = os.path.dirname(settings.BASE_DIR)
        landmarks_dir_parent = os.path.join(parent_dir, "sign_to_speech", "landmarks")
        module_path = os.path.join(landmarks_dir_parent, "gloss_to_speech.py")
        
        if not os.path.exists(module_path):
            script, _, _, _ = _paths()
            landmarks_dir_fallback = os.path.dirname(script)
            module_path = os.path.join(landmarks_dir_fallback, "gloss_to_speech.py")
            landmarks_dir = landmarks_dir_fallback
        else:
            landmarks_dir = landmarks_dir_parent
    ...
    try:
        if _CACHED_NLP_MOD:
            sentence = _CACHED_NLP_MOD.pure_nlp_processor(gloss_content)
        else:
...
def tts_speak(request):
    ...
    try:
        if _CACHED_SYNTH is None:
            parent_dir = os.path.dirname(settings.BASE_DIR)
            landmarks_dir = os.path.join(parent_dir, "sign_to_speech", "landmarks")
            if not os.path.exists(landmarks_dir):
                script, _, _, _ = _paths()
            ...
            _CACHED_SYNTH = create_synthesizer(rate=180)
            
        if not _CACHED_SYNTH:
            print("--- [TTS] Synthesizer could not be initialized ---")
            return JsonResponse({"error": "tts_unavailable"}, status=500)
            
        print(f"--- [TTS] Speaking: {text} ---")
        ok = _CACHED_SYNTH.speak(text)
        print(f"--- [TTS] Success: {ok} ---")
...
@csrf_exempt
def current_signs(request):
    if request.method != "GET":
        return JsonResponse({"error": "Invalid request"}, status=400)
    
    return JsonResponse({
        "recorded_signs": CURRENT_RECORDED_SIGNS,
